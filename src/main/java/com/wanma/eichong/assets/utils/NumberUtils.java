...
public class NumberUtils {
    ...
    public static void main(String[] args) {
        System.out.println(NumberUtils.numberCN2Arab("十六")+1);
        System.out.println(NumberUtils.numberKArab2CN(NumberUtils.numberCN2Arab("十六")+1));
    }
    ...
    public static int numberCN2Arab(String numberCN) {
        String tempNumberCN = numberCN;//异常数据处理;
         ...
         */
        String[] nums =new String[bigUnits.length+ 1]; //千位以内,直接处理;
        nums[0] = tempNumberCN;
        /*
         *分割大数字,以千为单位进行运算;
         */
        for(int i = (bigUnits.length- 1); i >= 0; i--) { //是否存在大单位(万,亿...);
            int find = tempNumberCN.indexOf(bigUnits[i]);
            if(find != -1) {
                String[] tempStrs = tempNumberCN.split(bigUnits[i]); //清空千位内容;
                if(nums[0] !=null) {
                    nums[0] =null;
                }
    ...
    public static String numberArab2CN(Integer num) {
        ...
        for(int i = 0; i < rempNumsLen; i++) {//小于1000补零处理;
            ...
            if(i < rempNumsLen - 1) {//加上单位(万,亿....)
                tempResultNum +=bigUnits[rempNumsLen - i - 2];
            }
        }

        tempResultNum = tempResultNum.replaceAll(numZero+"$","");//去掉未位的零
...
//去掉未位的零
    ...
    private static String numberKCN2Arab(String numberCN) {
        ...
        if(numberCN !=null) {
            ...
            char ones = numberCN.charAt(numberCN.length() - 1);//处理十位
            nums[nums.length- 1] =numberCharCN2Arab(ones);//处理个位
